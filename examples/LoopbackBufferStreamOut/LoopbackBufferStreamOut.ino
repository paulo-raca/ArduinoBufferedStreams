/* 
  This example shows the out_stream functionality connecting LoopbackStream to automatically output to a Stream

  The example only shows the advantage of using the LoopbackStream buffer if SERIAL_PORT is set to a Stream that doesn't use flow control
  ...
  and with USE_LOOPBACK_STREAM_BUFFER == 0 you should see dropped characters (do a diff on the echo'd output) because of the delay(50) in loop()
  but with USE_LOOPBACK_STREAM_BUFFER == 1 you should see the echo
...
#define USE_LOOPBACK_STREAM_BUFFER 1

// loopback outputs data written to it to SERIAL_PORT after flush() or flushAvailableToWrite() is called, and has a 4096 byte buffer
LoopbackStream loopback(SERIAL_PORT, 4096);

void setup() {
  SERIAL_PORT.begin(9600); // purposely set to a slow baud rate for this example
  delay(100);

  loopback.println("Hello, World");
}

void loop() {
  #if (USE_LOOPBACK_STREAM_BUFFER == 1)
    ...
    loopback.fillFromStream(SERIAL_PORT);
    loopback.flushAvailableForWrite(false); // this writes the buffered data to SERIAL_PORT until SERIAL_PORT.availableForWrite() == 0 (when writes would start blocking)
    //loopback.flush(); // we don't use the blocking flush() here as the delay while blocking could cause charcters to be lost
    //loopback.flushAvailableForWrite(); // we don't use flushAvailableForWrite() which defaults to flushAvailableForWrite(true) and would call the blocking SERIAL_PORT.flush() method
  #else
